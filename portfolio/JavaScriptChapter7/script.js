...
function displayCalendar(witchMonth) {
    var date;
    ...
    if (month === 0 || month === 2 || month === 4 || month === 6 || month === 7 || month === 9 || month === 11) {
        ...
        if (year % 4 === 0) {
            //year ending in 00 not leapyear unless divisable by 400
            if (year % 100 === 0) {
    ...
    for (var i = 0; i < dateCells.length; i++) {
        //clear existing table datedates
...
} //end displayCalendar(witchMonth)
...
function createEventListeners() {
    ...
    if (prevLink.addEventListener) {
        ...
        prevLink.addEventListener("onclick", prevMo);
        nextLink.addEventListener("onclick", nextMo);
    } //makes it to where i  can go throught the months in the block object that appears
